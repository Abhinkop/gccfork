...
{
    tree *fndecl_type_param = new tree[args.size()];
    for (size_t i = 0; i < args.size(); i++)
    {
        fndecl_type_param[i] = args[i];
    }
    tree fndecl_type = build_function_type_array(integer_type_node, 2, fndecl_type_param);
    ...
    tree bind_expr = build3(BIND_EXPR, void_type_node, NULL, stmt_list, current_block);
    DECL_SAVED_TREE(function) = bind_expr;
    DECL_INITIAL(function) = current_block;
    BLOCK_SUPERCONTEXT(current_block) = function;
    BLOCK_VARS(current_block) = BIND_EXPR_VARS(bind_expr);
...
void GenericFunction::addStatement(tree& stmt)
{
